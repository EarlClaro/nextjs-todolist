...
export default function Home() {
  ...
  return (
    <main className="bg-gray-800 min-h-screen w-screen flex flex-col items-center" style={{backgroundColor:"black"}}>
      ...
      <div className="mt-10 w-11/12 max-w-[800px] max-h-5/6 z-10">
        <h1 className="text-3xl md:text-5xl self-start m-6 " style={{color:"white"}}>Earl's To-Do List</h1>
        ...
        <div className={`flex justify-between items-center bg-slate-300 text-black rounded-t-none ${!isNewTaskActive && "rounded-xl border-b-2 border-black"} py-3 px-2 md:px-5 md:py-3 w-full hover:shadow-xl`} >
          <input className="bg-transparent w-full text-sm md:text-xl focus:outline-none placeholder:text-gray-800"  value={newTask} onChange={(e) => setNewTask(e.target.value)} onKeyDown={handleKeyDown} onFocus={()=>setIsNewTaskActive(true)} placeholder="Input your new Task"/>
        ...
        { isNewTaskActive && 
          <div className="flex justify-between items-center bg-slate-400 text-black rounded-t-none rounded-xl py-3 px-2 md:px-5 md:py-3 w-full hover:shadow-xl">
            <textarea className="bg-transparent w-full text-sm md:text-xl focus:outline-none placeholder:text-gray-800"  value={descriptionInput} onChange={(e)=>setDescriptionInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Input the description of the task"/>
          </div>}
      </div>
    </main>
  );
